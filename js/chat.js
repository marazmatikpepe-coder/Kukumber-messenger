...
var userAvatarCache = {};
var usernameCache = {};
// ========== ПРОСТОЕ ОТКРЫТИЕ ПРОФИЛЯ ГРУППЫ/КАНАЛА ==========
function openChannelOrGroupProfile() {
    ...
    // ПРИНУДИТЕЛЬНО проверяем тип
    const chatType = currentChatUser.type;
    
    if (chatType === 'group') {
        // ГРУППА - открываем групповой профиль
        showSimpleGroupProfile(currentChatUser);
    } else if (chatType === 'channel') {
        // КАНАЛ - открываем канальный профиль
        showSimpleChannelProfile(currentChatUser);
    } else {
        showNotification('Неизвестный тип чата', 'error');
    }
}

// ПРОСТОЙ ПРОФИЛЬ ГРУППЫ (без лишних функций)
function showSimpleGroupProfile(chat) {
    ...
    const canEdit = isAdmin || isOwner || window.isSuperAdmin;
    
    const modalHtml = `
        <div id="simple-group-profile" class="modal" style="z-index: 10001;">
            <div style="background: white; width: 100%; max-width: 450px; border-radius: 24px; overflow: hidden; margin: auto; max-height: 90vh; overflow-y: auto;">
                <div style="background: linear-gradient(135deg, #228B22, #556B2F); height: 120px; position: relative;">
                    <button onclick="closeSimpleGroupProfile()" style="position: absolute; top: 10px; right: 15px; background: rgba(0,0,0,0.5); color: white; border: none; width: 32px; height: 32px; border-radius: 50%; font-size: 20px; cursor: pointer;">×</button>
                </div>
                <div style="display: flex; justify-content: center; margin-top: -50px;">
                    <div style="width: 90px; height: 90px; border-radius: 50%; background: var(--sage); border: 4px solid white; display: flex; align-items: center; justify-content: center; font-size: 45px; ${chat.avatar ? `background-image: url(${chat.avatar}); background-size: cover;` : ''}">
                        ${chat.avatar ? '' : '👥'}
                    </div>
                </div>
                <div style="text-align: center; padding: 15px;">
                    <h2 style="margin: 0;">${escapeHtml(chat.name || 'Группа')}</h2>
                    <div style="color: gray; font-size: 14px; margin-top: 5px;">👥 ${membersCount} участников</div>
                    <p style="color: #666; font-size: 14px; margin-top: 10px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                    <div style="margin-top: 15px; padding: 10px 0; border-top: 1px solid #eee;">
                        <span style="font-size: 13px; color: gray;">${chat.isPublic ? '🌍 Публичная группа' : '🔒 Приватная группа'}</span>
                    </div>
                </div>
                <div style="display: flex; border-top: 1px solid #eee;">
                    <button class="group-profile-tab active" data-tab="members" style="flex:1; padding: 12px; background: none; border: none; cursor: pointer;">👥 Участники</button>
                    <button class="group-profile-tab" data-tab="info" style="flex:1; padding: 12px; background: none; border: none; cursor: pointer;">ℹ️ Инфо</button>
                    ${canEdit ? `<button class="group-profile-tab" data-tab="admin" style="flex:1; padding: 12px; background: none; border: none; cursor: pointer;">⚙️ Управление</button>` : ''}
                </div>
                <div id="group-profile-content" style="padding: 15px; min-height: 200px;"></div>
                <div style="padding: 15px; border-top: 1px solid #eee;">
                    <button id="group-profile-action" style="width: 100%; padding: 12px; border-radius: 30px; border: none; background: ${isMember ? '#dc3545' : '#228B22'}; color: white; font-size: 16px; cursor: pointer;">
    ...
    const old = document.getElementById('simple-group-profile');
    if (old) old.remove();
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    document.getElementById('simple-group-profile').classList.remove('hidden');
    
    // Загружаем участников
    loadSimpleGroupMembers(chat);
    
    // Обработчики вкладок
    document.querySelectorAll('.group-profile-tab').forEach(btn => {
        btn.onclick = () => {
            document.querySelectorAll('.group-profile-tab').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            const tab = btn.dataset.tab;
            if (tab === 'members') loadSimpleGroupMembers(chat);
            else if (tab === 'info') loadSimpleGroupInfo(chat);
            else if (tab === 'admin') loadSimpleGroupAdmin(chat);
        };
    });
    
    // Кнопка действия
    const actionBtn = document.getElementById('group-profile-action');
    if (actionBtn) {
        actionBtn.onclick = () => {
            if (isMember) {
                database.ref('chats/' + currentChatId + '/members/' + currentUser.uid).remove()
                    .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).remove())
                    .then(() => { showNotification('Вы покинули группу', 'info'); closeSimpleGroupProfile(); closeChat(); loadChats(); });
            } else {
                database.ref('chats/' + currentChatId + '/members/' + currentUser.uid).set(true)
                    .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).set(true))
                    .then(() => { showNotification('Вы вступили в группу!', 'success'); closeSimpleGroupProfile(); });
            }
        };
    }
}

function loadSimpleGroupMembers(chat) {
    const container = document.getElementById('group-profile-content');
    if (!container) return;
    ...
    container.innerHTML = '<div id="simple-members-list"></div>';
    const membersContainer = document.getElementById('simple-members-list');
    ...
    memberIds.forEach(uid => {
        database.ref('users/' + uid).once('value', userSnap => {
            ...
            let role = 'участник';
            if (uid === ownerId) role = '👑 владелец';
            else if (admins[uid]) role = '⚙️ админ';
            
            const div = document.createElement('div');
            div.style.cssText = 'display: flex; align-items: center; gap: 12px; padding: 10px; border-bottom: 1px solid #eee; cursor: pointer;';
            div.onclick = () => openUserProfile(uid);
            div.innerHTML = `
                <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--sage); display: flex; align-items: center; justify-content: center; ${user.avatar ? `background-image: url(${user.avatar}); background-size: cover;` : ''}">${!user.avatar ? '👤' : ''}</div>
                <div><strong>${escapeHtml(user.username)}</strong><div style="font-size: 11px; color: #666;">${role}</div></div>
            `;
...
function loadSimpleGroupInfo(chat) {
    const container = document.getElementById('group-profile-content');
    if (!container) return;
    ...
    database.ref('users/' + chat.createdBy).once('value', ownerSnap => {
        ...
        container.innerHTML = `
            <div><strong>📅 Дата создания</strong><br><span style="color:gray;">${chat.createdAt ? new Date(chat.createdAt).toLocaleDateString() : 'Неизвестно'}</span></div>
            <div style="margin-top: 15px;"><strong>👑 Владелец</strong><br><span style="color:gray; cursor:pointer;" onclick="openUserProfile('${chat.createdBy}')">${owner ? escapeHtml(owner.username) : 'Неизвестен'}</span></div>
            <div style="margin-top: 15px;"><strong>🌍 Тип</strong><br><span style="color:gray;">${chat.isPublic ? 'Публичная' : 'Приватная'}</span></div>
        `;
    });
}

function loadSimpleGroupAdmin(chat) {
    const container = document.getElementById('group-profile-content');
    if (!container) return;
    container.innerHTML = `<div style="text-align:center; padding:20px;">⚙️ Настройки группы (в разработке)</div>`;
}

function closeSimpleGroupProfile() {
    const modal = document.getElementById('simple-group-profile');
    if (modal) modal.remove();
}

// ПРОСТОЙ ПРОФИЛЬ КАНАЛА
function showSimpleChannelProfile(chat) {
    ...
    const modalHtml = `
        <div id="simple-channel-profile" class="modal" style="z-index: 10001;">
            <div style="background: white; width: 100%; max-width: 450px; border-radius: 24px; overflow: hidden; margin: auto;">
                <div style="background: linear-gradient(135deg, #228B22, #556B2F); height: 120px; position: relative;">
                    <button onclick="closeSimpleChannelProfile()" style="position: absolute; top: 10px; right: 15px; background: rgba(0,0,0,0.5); color: white; border: none; width: 32px; height: 32px; border-radius: 50%; font-size: 20px; cursor: pointer;">×</button>
                </div>
                <div style="display: flex; justify-content: center; margin-top: -50px;">
                    <div style="width: 90px; height: 90px; border-radius: 50%; background: var(--sage); border: 4px solid white; display: flex; align-items: center; justify-content: center; font-size: 45px; ${chat.avatar ? `background-image: url(${chat.avatar}); background-size: cover;` : ''}">
                        ${chat.avatar ? '' : '📢'}
                ...
                <div style="text-align: center; padding: 15px;">
                    <h2 style="margin: 0;">${escapeHtml(chat.name || 'Канал')}</h2>
                    ${chat.kname ? `<div style="color: gray; font-size: 13px;">@${escapeHtml(chat.kname)}</div>` : ''}
                    <div style="color: gray; font-size: 14px; margin-top: 5px;">👥 ${membersCount} подписчиков</div>
                    <p style="color: #666; font-size: 14px; margin-top: 10px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                </div>
                <div style="padding: 15px; border-top: 1px solid #eee;">
                    <button id="channel-profile-action" style="width: 100%; padding: 12px; border-radius: 30px; border: none; background: ${isSubscribed ? '#dc3545' : '#228B22'}; color: white; font-size: 16px; cursor: pointer;">
    ...
    if (old) old.remove();
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    document.getElementById('simple-channel-profile').classList.remove('hidden');
    
    const actionBtn = document.getElementById('channel-profile-action');
    if (actionBtn) {
        actionBtn.onclick = () => {
            if (isSubscribed) {
                database.ref('chats/' + currentChatId + '/subscribers/' + currentUser.uid).remove()
                    .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).remove())
                    .then(() => { showNotification('Вы отписались', 'info'); closeSimpleChannelProfile(); loadChats(); });
            } else {
                database.ref('chats/' + currentChatId + '/subscribers/' + currentUser.uid).set(true)
                    .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).set(true))
                    .then(() => { showNotification('Вы подписались!', 'success'); closeSimpleChannelProfile(); });
            }
        };
    }
}
...
// ========== КРАСИВЫЙ ПРОФИЛЬ КАНАЛА/ГРУППЫ ==========
function openChannelOrGroupProfile() {
    if (!currentChatId || !currentChatUser) {
        showNotification('Чат не выбран', 'error');
        return;
    ...
    let modalHtml = `
        <div id="beauty-channel-profile" class="modal" style="z-index: 10001;">
            <div class="profile-modal-content" style="max-width: 500px; border-radius: 24px; overflow: hidden;">
                <div class="profile-banner" style="background: linear-gradient(135deg, #228B22, #556B2F); height: 120px; position: relative;">
                    <button class="profile-close-btn" onclick="closeBeautyChannelProfile()" style="position: absolute; top: 10px; right: 15px; background: rgba(0,0,0,0.5); color: white; border: none; width: 32px; height: 32px; border-radius: 50%; font-size: 18px; cursor: pointer;">×</button>
                </div>
                <div class="profile-avatar-wrapper" style="display: flex; justify-content: center; margin-top: -40px;">
                    <div class="profile-avatar" style="width: 80px; height: 80px; border-radius: 50%; background: var(--sage); border: 3px solid white; display: flex; align-items: center; justify-content: center; font-size: 40px; background-size: cover; background-position: center;">
                        ${chat.avatar ? '' : (isChannel ? '📢' : '👥')}
                    </div>
                </div>
                <div class="profile-info" style="text-align: center; padding: 15px;">
                    <div class="profile-name-row" style="display: flex; align-items: center; justify-content: center; gap: 10px; flex-wrap: wrap;">
                        <h2 class="profile-name" style="font-size: 22px; color: var(--text-dark);">${escapeHtml(chat.name || (isChannel ? 'Канал' : 'Группа'))}</h2>
                    </div>
                    ${chat.kname ? `<div class="profile-username" style="color: var(--text-muted); font-size: 14px; margin-top: 5px;">@${escapeHtml(chat.kname)}</div>` : ''}
                    <div class="profile-subscribers" style="font-size: 13px; color: var(--text-muted); margin-top: 5px;">👥 ${membersCount} ${isChannel ? 'подписчиков' : 'участников'}</div>
                    <div class="profile-status" style="font-size: 12px; color: var(--text-muted); margin-top: 5px;">${isChannel ? (chat.isPublic ? '🌍 Публичный канал' : '🔒 Приватный канал') : (chat.isPublic ? '🌍 Публичная группа' : '🔒 Приватная группа')}</div>
                    <p class="profile-bio" style="color: var(--text-light); margin-top: 8px; font-size: 14px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                </div>
                
                <div class="profile-tabs" style="display: flex; border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);">
                    <button class="profile-tab-btn active" onclick="switchBeautyChannelTab('info')" style="flex: 1; padding: 12px; background: none; border: none; cursor: pointer; font-size: 14px;">ℹ️ Информация</button>
                    <button class="profile-tab-btn" onclick="switchBeautyChannelTab('members')" style="flex: 1; padding: 12px; background: none; border: none; cursor: pointer; font-size: 14px;">👥 Участники</button>
                </div>
                
                <div id="beauty-channel-content" class="profile-content" style="padding: 15px; max-height: 400px; overflow-y: auto;">
                    <div class="profile-loading">Загрузка...</div>
                </div>
                
                <div class="profile-actions" style="padding: 15px; border-top: 1px solid var(--border); display: flex; gap: 10px;">
                    <button id="beauty-subscribe-btn" class="btn-primary" style="flex: 1;">${isChannel ? (chat.subscribers?.[currentUser.uid] ? 'Отписаться' : 'Подписаться') : (chat.members?.[currentUser.uid] ? 'Покинуть' : 'Вступить')}</button>
                </div>
            </div>
        </div>
    `;
    ...
    const avatarDiv = modal.querySelector('.profile-avatar');
    if (chat.avatar) {
        avatarDiv.style.backgroundImage = `url(${chat.avatar})`;
        avatarDiv.style.backgroundSize = 'cover';
        avatarDiv.textContent = '';
    ...
    // Настройка кнопки
    const actionBtn = document.getElementById('beauty-subscribe-btn');
    if (actionBtn) {
        if (isChannel) {
            const isSubscribed = chat.subscribers && chat.subscribers[currentUser.uid];
            actionBtn.textContent = isSubscribed ? 'Отписаться' : 'Подписаться';
            actionBtn.onclick = () => {
                if (isSubscribed) {
                    database.ref('chats/' + currentChatId + '/subscribers/' + currentUser.uid).remove()
                        .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).remove())
                        .then(() => { showNotification('Вы отписались', 'info'); closeBeautyChannelProfile(); loadChats(); });
                } else {
                    database.ref('chats/' + currentChatId + '/subscribers/' + currentUser.uid).set(true)
                        .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).set(true))
                        .then(() => { showNotification('Вы подписались!', 'success'); closeBeautyChannelProfile(); });
                }
            };
        } else {
            const isMember = chat.members && chat.members[currentUser.uid];
            actionBtn.textContent = isMember ? 'Покинуть группу' : 'Вступить';
            actionBtn.onclick = () => {
                if (isMember) {
                    database.ref('chats/' + currentChatId + '/members/' + currentUser.uid).remove()
                        .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).remove())
                        .then(() => { showNotification('Вы покинули группу', 'info'); closeBeautyChannelProfile(); closeChat(); loadChats(); });
                } else {
                    database.ref('chats/' + currentChatId + '/members/' + currentUser.uid).set(true)
                        .then(() => database.ref('userChats/' + currentUser.uid + '/' + currentChatId).set(true))
                        .then(() => { showNotification('Вы вступили в группу!', 'success'); closeBeautyChannelProfile(); });
                }
            };
        }
    }
    ...
    loadBeautyChannelInfo();
}

function loadBeautyChannelInfo() {
    const container = document.getElementById('beauty-channel-content');
    if (!container) return;
    ...
    let infoHtml = `
        <div style="padding: 10px;">
            <div style="margin-bottom: 15px;">
                ...
                <p style="margin-top: 5px; color: var(--text-muted);">${escapeHtml(chat.description || 'Нет описания')}</p>
            </div>
            <div style="margin-bottom: 15px;">
                <strong>🔗 Тип</strong>
                <p style="margin-top: 5px; color: var(--text-muted);">${isChannel ? (chat.isPublic ? 'Публичный канал' : 'Приватный канал') : (chat.isPublic ? 'Публичная группа' : 'Приватная группа')}</p>
            </div>
            ${chat.kname ? `
            <div style="margin-bottom: 15px;">
                <strong>🔖 K-name</strong>
                <p style="margin-top: 5px; color: var(--text-muted);">@${escapeHtml(chat.kname)}</p>
            </div>
            ` : ''}
            <div style="margin-bottom: 15px;">
                <strong>📅 Создан</strong>
                <p style="margin-top: 5px; color: var(--text-muted);">${chat.createdAt ? new Date(chat.createdAt).toLocaleDateString() : 'Неизвестно'}</p>
            </div>
        </div>
    `;
    ...
    document.querySelectorAll('#beauty-channel-profile .profile-tab-btn').forEach(btn => btn.classList.remove('active'));
    document.querySelector('#beauty-channel-profile .profile-tab-btn:first-child').classList.add('active');
}

function switchBeautyChannelTab(tab) {
    const container = document.getElementById('beauty-channel-content');
    if (!container) return;
    ...
    // Подсветка кнопок
    document.querySelectorAll('#beauty-channel-profile .profile-tab-btn').forEach((btn, i) => {
        btn.classList.remove('active');
        if ((tab === 'info' && i === 0) || (tab === 'members' && i === 1)) {
            btn.classList.add('active');
        }
    });
    ...
    if (tab === 'info') {
        ...
        memberIds.forEach(uid => {
            database.ref('users/' + uid).once('value').then(userSnap => {
                ...
                div.onclick = () => openUserProfile(uid);
                div.innerHTML = `
                    <div class="avatar" style="background-image: url(${user.avatar || ''}); background-size: cover; width:40px; height:40px; border-radius:50%;">${!user.avatar ? '👤' : ''}</div>
                    <div style="flex:1;"><strong>${escapeHtml(user.username)}</strong></div>
                `;
                container.appendChild(div);
            });
        ...
        <div id="simple-channel-profile" class="modal" style="z-index: 10001;">
            <div style="background: white; width: 90%; max-width: 400px; border-radius: 20px; overflow: hidden; margin: auto;">
                ...
                <div style="text-align: center; padding: 15px;">
                    ...
                    <div style="color: gray; font-size: 12px; margin-top: 5px;">${isChannel ? (chat.isPublic ? '🌍 Публичный' : '🔒 Приватный') : (chat.isPublic ? '🌍 Публичная' : '🔒 Приватная')}</div>
                    <p style="color: #666; font-size: 14px; margin-top: 10px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                </div>
                <div style="padding: 15px; border-top: 1px solid #eee;">
                    <button id="simple-action-btn" style="width: 100%; padding: 12px; border: none; border-radius: 30px; background: #228B22; color: white; font-size: 16px; cursor: pointer;">
...
let currentProfileChatData = null;
...
function openChannelOrGroupProfile() {
    ...
    currentProfileChatId = currentChatId;
    currentProfileChatData = currentChatUser;
    
    // Загружаем свежие данные
    database.ref('chats/' + currentChatId).once('value').then(snapshot => {
        const chat = snapshot.val();
        if (!chat) return;
        currentProfileChatData = chat;
...
function showBeautyChannelProfile(chat) {
        ...
        <div id="beauty-channel-profile" class="modal" style="z-index: 10001;">
            <div style="background: white; width: 100%; max-width: 500px; border-radius: 24px; overflow: hidden; margin: auto; position: relative; max-height: 90vh; overflow-y: auto;">
                ...
                <div style="${bannerStyle} height: 140px; position: relative;">
                    ${canEdit ? `<button class="edit-banner-btn" onclick="editChannelBannerNew()" style="position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.6); border: none; color: white; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; font-size: 16px;">✏️</button>` : ''}
                ...
                <div style="display: flex; justify-content: center; margin-top: -50px; position: relative; z-index: 2;">
                    <div style="width: 100px; height: 100px; border-radius: 50%; background: var(--sage); border: 4px solid white; display: flex; align-items: center; justify-content: center; font-size: 50px; ${chat.avatar ? `background-image: url(${chat.avatar}); background-size: cover;` : ''}">
                        ${chat.avatar ? '' : (isChannel ? '📢' : '👥')}
                        ${canEdit ? `<button onclick="editChannelAvatarNew()" style="position: absolute; bottom: 5px; right: 5px; background: var(--forest); border: none; color: white; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; font-size: 14px;">✏️</button>` : ''}
                    </div>
                </div>
                ...
                <div style="text-align: center; padding: 15px 20px;">
                    <div style="display: flex; align-items: center; justify-content: center; gap: 8px; flex-wrap: wrap;">
                        <h2 style="font-size: 22px; margin: 0;">${escapeHtml(chat.name || (isChannel ? 'Канал' : 'Группа'))}</h2>
                        ${chat.verified ? '<img src="https://i.ibb.co/YTRCNHkq/4e9cba55-b083-46d3-8a30-bff7b1be94c7-1.png" style="width: 20px; height: 20px;">' : ''}
                    </div>
                    ...
                    <!-- Статистика и кнопки -->
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 15px; padding: 10px 0; border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);">
                        <div style="text-align: center; flex: 1;">
                            <div style="font-size: 18px; font-weight: bold;" id="profile-members-count">${membersCount}</div>
                            <div style="font-size: 12px; color: var(--text-muted);">${isChannel ? 'подписчиков' : 'участников'}</div>
                        </div>
                        <div style="flex: 2; display: flex; gap: 10px; justify-content: center;">
                            <button id="profile-action-btn" style="padding: 8px 24px; border: none; border-radius: 30px; background: var(--forest); color: white; font-size: 14px; cursor: pointer;">
                                ${isChannel ? (isSubscribed ? 'Отписаться' : 'Подписаться') : (isMember ? 'Покинуть' : 'Вступить')}
                            </button>
                            ${!isChannel ? '' : `
                                <button id="profile-notify-btn" onclick="toggleChannelNotifications()" style="width: 40px; height: 40px; border-radius: 50%; border: none; background: var(--background); cursor: pointer; font-size: 20px;">
                                    🔔
                                </button>
                            `}
                        </div>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 10px; font-size: 13px; color: var(--text-muted);">
                        <span>${isChannel ? (chat.isPublic ? '🌍 Публичный канал' : '🔒 Приватный канал') : (chat.isPublic ? '🌍 Публичная группа' : '🔒 Приватная группа')}</span>
                        ${isChannel && chat.category ? `<span>${categories[chat.category] || chat.category}</span>` : ''}
                    </div>
                </div>
                
                <!-- Вкладки -->
                <div style="display: flex; border-top: 1px solid var(--border); background: white;">
                    <button class="profile-tab-btn-new active" data-tab="posts" style="flex: 1; padding: 14px; background: none; border: none; cursor: pointer; font-size: 14px; transition: all 0.2s;">📷 Посты</button>
                    <button class="profile-tab-btn-new" data-tab="info" style="flex: 1; padding: 14px; background: none; border: none; cursor: pointer; font-size: 14px; transition: all 0.2s;">ℹ️ Инфо</button>
                    ${canEdit ? `<button class="profile-tab-btn-new" data-tab="admin" style="flex: 1; padding: 14px; background: none; border: none; cursor: pointer; font-size: 14px; transition: all 0.2s;">⚙️ Управление</button>` : ''}
                </div>
                
                <!-- Контент вкладок -->
                <div id="profile-tab-content" style="padding: 15px; min-height: 200px; max-height: 400px; overflow-y: auto;">
                    <div class="profile-loading">Загрузка...</div>
                </div>
            </div>
        </div>
    ...
    const avatarDiv = modal.querySelector('.avatar-img');
    
    // Подписываемся на обновление количества подписчиков
    const membersRef = isChannel ? 'subscribers' : 'members';
    database.ref('chats/' + currentProfileChatId + '/' + membersRef).on('value', (snap) => {
        const count = Object.keys(snap.val() || {}).length;
        const countEl = document.getElementById('profile-members-count');
        if (countEl) countEl.textContent = count;
    });
    
    // Обработчики вкладок
    document.querySelectorAll('.profile-tab-btn-new').forEach(btn => {
        btn.onclick = () => {
            document.querySelectorAll('.profile-tab-btn-new').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            const tab = btn.dataset.tab;
            if (tab === 'posts') loadProfilePosts();
            else if (tab === 'info') loadProfileInfo();
            else if (tab === 'admin') loadProfileAdmin();
        };
    });
    
    // Кнопка действия (подписаться/отписаться)
    const actionBtn = document.getElementById('profile-action-btn');
    if (actionBtn) {
        actionBtn.onclick = () => handleProfileAction();
    }
...
function handleProfileAction() {
    const chat = currentProfileChatData;
    const isChannel = chat.type === 'channel';
    const isSubscribed = chat.subscribers ? chat.subscribers[currentUser.uid] : false;
    const isMember = chat.members ? chat.members[currentUser.uid] : false;
    
    if (isChannel) {
        const ref = database.ref('chats/' + currentProfileChatId + '/subscribers/' + currentUser.uid);
        if (isSubscribed) {
            ref.remove().then(() => {
                database.ref('userChats/' + currentUser.uid + '/' + currentProfileChatId).remove();
                showNotification('Вы отписались от канала', 'info');
                updateProfileActionButton();
            });
        } else {
            ref.set(true).then(() => {
                database.ref('userChats/' + currentUser.uid + '/' + currentProfileChatId).set(true);
                showNotification('Вы подписались на канал!', 'success');
                updateProfileActionButton();
            });
        }
    } else {
        const ref = database.ref('chats/' + currentProfileChatId + '/members/' + currentUser.uid);
        if (isMember) {
            ref.remove().then(() => {
                database.ref('userChats/' + currentUser.uid + '/' + currentProfileChatId).remove();
                showNotification('Вы покинули группу', 'info');
...
function updateProfileActionButton() {
    database.ref('chats/' + currentProfileChatId).once('value').then(snapshot => {
        const chat = snapshot.val();
        if (!chat) return;
        currentProfileChatData = chat;
        ...
        const actionBtn = document.getElementById('profile-action-btn');
        if (actionBtn) {
            actionBtn.textContent = isChannel ? (isSubscribed ? 'Отписаться' : 'Подписаться') : (isMember ? 'Покинуть' : 'Вступить');
        }
    });
}
...
function loadProfilePosts() {
    ...
    database.ref('slices').orderByChild('channelId').equalTo(currentProfileChatId).once('value', snapshot => {
        const slices = snapshot.val();
        container.innerHTML = '';
        ...
        posts.forEach(post => {
            ...
            div.innerHTML = `
                <div style="padding: 12px;">
                    <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 8px;">${formatSliceDateNew(post.data.createdAt)}</div>
...
function loadProfileInfo() {
    ...
    // Загружаем владельца
    let ownerHtml = '';
    database.ref('users/' + chat.createdBy).once('value', (snap) => {
        const owner = snap.val();
        ownerHtml = `<div style="margin-bottom: 12px;">
            <strong>👑 Владелец</strong><br>
            <span style="color: var(--text-muted);">${owner ? escapeHtml(owner.username) : 'Неизвестен'}</span>
        </div>`;
        ...
        if (chat.admins) {
            ...
            if (adminIds.length > 0) {
                adminsHtml = '<div style="margin-bottom: 12px;"><strong>👥 Администраторы</strong><br><div style="margin-top: 5px;">';
                adminIds.forEach(uid => {
                    database.ref('users/' + uid).once('value', (userSnap) => {
                        const admin = userSnap.val();
                        if (admin) {
                            const adminDiv = document.createElement('div');
                            adminDiv.style.margin = '5px 0';
                            adminDiv.innerHTML = `<span style="color: var(--text-muted);">${escapeHtml(admin.username)}</span>`;
                            document.getElementById('profile-admins-list')?.appendChild(adminDiv);
                        }
                    });
                });
                adminsHtml += '<div id="profile-admins-list"></div></div>';
            }
        }
        
        const inviteLink = `${window.location.origin}${window.location.pathname}?join=${chat.kname || chat.id}`;
        ...
        const infoHtml = `
            <div style="padding: 5px;">
                ...
                <div style="margin-top: 15px; padding-top: 10px; border-top: 1px solid var(--border);">
                    <strong>🔗 Ссылка-приглашение</strong><br>
                    <div style="display: flex; gap: 8px; margin-top: 5px;">
                        <input type="text" value="${inviteLink}" readonly style="flex: 1; padding: 8px; border: 1px solid var(--border); border-radius: 8px; font-size: 12px;">
                        <button onclick="copyInviteLink('${inviteLink}')" style="padding: 8px 16px; background: var(--forest); color: white; border: none; border-radius: 8px; cursor: pointer;">Копировать</button>
                    </div>
                </div>
            </div>
        `;
    ...
    container.innerHTML = `
        <div style="margin-bottom: 15px;">
            ...
            <div style="margin-top: 10px;">
                <button onclick="editChannelInfoNew()" style="width: 100%; padding: 12px; margin-bottom: 8px; background: var(--background); border: none; border-radius: 12px; text-align: left; cursor: pointer;">✏️ Редактировать название и описание</button>
                <button onclick="editChannelCategoryNew()" style="width: 100%; padding: 12px; margin-bottom: 8px; background: var(--background); border: none; border-radius: 12px; text-align: left; cursor: pointer;">🏷️ Изменить тематику</button>
            </div>
        </div>
        <div style="margin-bottom: 15px;">
            <strong>👥 Управление подписчиками</strong>
            <div id="admin-members-list" style="margin-top: 10px; max-height: 250px; overflow-y: auto;">
                <div class="profile-loading">Загрузка...</div>
            </div>
        </div>
        ${chat.createdBy === currentUser.uid || window.isSuperAdmin ? `
        <div style="margin-top: 15px; padding-top: 10px; border-top: 1px solid var(--border);">
            <button onclick="deleteChannelNew()" style="width: 100%; padding: 12px; background: #dc3545; color: white; border: none; border-radius: 12px; cursor: pointer;">🗑️ Удалить ${isChannel ? 'канал' : 'группу'}</button>
        </div>
        ` : ''}
    `;
        ...
        memberIds.forEach(uid => {
            database.ref('users/' + uid).once('value', userSnap => {
                ...
                div.innerHTML = `
                    ...
                    ${uid !== chat.createdBy ? `<button onclick="removeMemberFromChannel('${uid}')" style="background: none; border: none; color: #dc3545; font-size: 20px; cursor: pointer;">🗑️</button>` : '<span style="color: var(--text-muted);">👑 Владелец</span>'}
                `;
...
function removeMemberFromChannel(uid) {
    if (!confirm('Удалить этого участника?')) return;
    const ref = currentProfileChatData.type === 'channel' ? 'subscribers' : 'members';
    database.ref('chats/' + currentProfileChatId + '/' + ref + '/' + uid).remove().then(() => {
        database.ref('userChats/' + uid + '/' + currentProfileChatId).remove();
        showNotification('Участник удалён', 'success');
        loadProfileAdmin();
    });
}

function editChannelInfoNew() {
    const newName = prompt('Новое название:', currentProfileChatData.name);
    const newDesc = prompt('Новое описание:', currentProfileChatData.description || '');
    if (newName) {
        database.ref('chats/' + currentProfileChatId).update({
            name: newName.trim(),
...
function editChannelCategoryNew() {
    ...
    let options = '';
    categories.forEach(c => { options += `<option value="${c}" ${currentProfileChatData.category === c ? 'selected' : ''}>${catNames[c]}</option>`; });
    ...
    const newCat = prompt('Выберите тематику:', selectHtml);
    if (newCat && categories.includes(newCat)) {
        database.ref('chats/' + currentProfileChatId).update({ category: newCat }).then(() => {
...
function editChannelBannerNew() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            await database.ref('users/' + currentProfileChatId).update({ banner: url });
...
function editChannelAvatarNew() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            const url = await uploadToImgBB(file);
            await database.ref('chats/' + currentProfileChatId).update({ avatar: url });
...
function deleteChannelNew() {
    if (!confirm('Удалить навсегда? Это действие необратимо!')) return;
    database.ref('chats/' + currentProfileChatId).remove().then(() => {
        database.ref('messages/' + currentProfileChatId).remove();
...
function toggleChannelNotifications() {
    const ref = database.ref('channelNotifications/' + currentUser.uid + '/' + currentProfileChatId);
...
function closeBeautyChannelProfile() {
    const modal = document.getElementById('beauty-channel-profile');
    if (modal) modal.remove();
    if (currentProfileChatId) {
        database.ref('chats/' + currentProfileChatId + '/subscribers').off();
        database.ref('chats/' + currentProfileChatId + '/members').off();
    }
...
// ========== ПРОФИЛЬ ГРУППЫ (ДОБАВИТЬ В КОНЕЦ ФАЙЛА) ==========
function openChannelOrGroupProfile() {
    if (!currentChatId || !currentChatUser) {
        showNotification('Чат не выбран', 'error');
        return;
    ...
    if (currentChatUser.type === 'group') {
        ...
        openChannelProfileSimple(currentChatId);
...
function openGroupProfile(chatId) {
    database.ref('chats/' + chatId).once('value').then(snapshot => {
        ...
        const modalHtml = `
            <div id="group-profile-modal" class="modal" style="z-index: 10001;">
                <div style="background: white; width: 100%; max-width: 400px; border-radius: 24px; overflow: hidden; margin: auto;">
                    <div style="background: linear-gradient(135deg, #228B22, #556B2F); height: 100px; position: relative;">
                        <button onclick="closeGroupProfileModal()" style="position: absolute; top: 10px; right: 15px; background: rgba(0,0,0,0.5); color: white; border: none; width: 30px; height: 30px; border-radius: 50%; cursor: pointer;">×</button>
                    </div>
                    <div style="text-align: center; margin-top: -40px;">
                        <div style="width: 80px; height: 80px; border-radius: 50%; background: var(--sage); margin: 0 auto; border: 3px solid white; display: flex; align-items: center; justify-content: center; font-size: 40px; ${chat.avatar ? `background-image: url(${chat.avatar}); background-size: cover;` : ''}">
                            ${chat.avatar ? '' : '👥'}
                    ...
                    <div style="text-align: center; padding: 15px;">
                        <h2 style="margin: 0;">${escapeHtml(chat.name || 'Группа')}</h2>
                        <div style="color: gray; margin-top: 5px;">👥 ${membersCount} участников</div>
                        <p style="color: #666; margin-top: 10px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                    </div>
                    <div style="padding: 15px; border-top: 1px solid #eee;">
                        <button onclick="handleGroupAction('${chatId}', ${isMember})" style="width: 100%; padding: 12px; border-radius: 30px; border: none; background: ${isMember ? '#dc3545' : '#228B22'}; color: white; cursor: pointer;">
                            ${isMember ? 'Покинуть группу' : 'Вступить в группу'}
                        </button>
                    </div>
                </div>
        ...
        const old = document.getElementById('group-profile-modal');
        if (old) old.remove();
        document.body.insertAdjacentHTML('beforeend', modalHtml);
        document.getElementById('group-profile-modal').classList.remove('hidden');
    });
}

function handleGroupAction(chatId, isMember) {
    if (isMember) {
        database.ref('chats/' + chatId + '/members/' + currentUser.uid).remove()
            .then(() => database.ref('userChats/' + currentUser.uid + '/' + chatId).remove())
            .then(() => { showNotification('Вы покинули группу', 'info'); closeGroupProfileModal(); closeChat(); loadChats(); });
    } else {
        database.ref('chats/' + chatId + '/members/' + currentUser.uid).set(true)
            .then(() => database.ref('userChats/' + currentUser.uid + '/' + chatId).set(true))
            .then(() => { showNotification('Вы вступили в группу!', 'success'); closeGroupProfileModal(); });
    }
}

function openChannelProfileSimple(chatId) {
    database.ref('chats/' + chatId).once('value').then(snapshot => {
        const chat = snapshot.val();
        ...
        const modalHtml = `
            <div id="channel-profile-modal-simple" class="modal" style="z-index: 10001;">
                <div style="background: white; width: 100%; max-width: 400px; border-radius: 24px; overflow: hidden; margin: auto;">
                    <div style="background: linear-gradient(135deg, #228B22, #556B2F); height: 100px; position: relative;">
                        <button onclick="closeChannelProfileModalSimple()" style="position: absolute; top: 10px; right: 15px; background: rgba(0,0,0,0.5); color: white; border: none; width: 30px; height: 30px; border-radius: 50%; cursor: pointer;">×</button>
                    </div>
                    <div style="text-align: center; margin-top: -40px;">
                        <div style="width: 80px; height: 80px; border-radius: 50%; background: var(--sage); margin: 0 auto; border: 3px solid white; display: flex; align-items: center; justify-content: center; font-size: 40px; ${chat.avatar ? `background-image: url(${chat.avatar}); background-size: cover;` : ''}">
                            ${chat.avatar ? '' : '📢'}
                    ...
                    <div style="text-align: center; padding: 15px;">
                        <h2 style="margin: 0;">${escapeHtml(chat.name || 'Канал')}</h2>
                        ${chat.kname ? `<div style="color: gray; font-size: 13px;">@${escapeHtml(chat.kname)}</div>` : ''}
                        <div style="color: gray; margin-top: 5px;">👥 ${membersCount} подписчиков</div>
                        <p style="color: #666; margin-top: 10px;">${escapeHtml(chat.description || 'Нет описания')}</p>
                    </div>
                    <div style="padding: 15px; border-top: 1px solid #eee;">
                        <button onclick="handleChannelAction('${chatId}', ${isSubscribed})" style="width: 100%; padding: 12px; border-radius: 30px; border: none; background: ${isSubscribed ? '#dc3545' : '#228B22'}; color: white; cursor: pointer;">
                            ${isSubscribed ? 'Отписаться' : 'Подписаться'}
                        </button>
                    </div>
                </div>
        ...
        const old = document.getElementById('channel-profile-modal-simple');
        if (old) old.remove();
        document.body.insertAdjacentHTML('beforeend', modalHtml);
        document.getElementById('channel-profile-modal-simple').classList.remove('hidden');
    });
}

function handleChannelAction(chatId, isSubscribed) {
    if (isSubscribed) {
        database.ref('chats/' + chatId + '/subscribers/' + currentUser.uid).remove()
            .then(() => database.ref('userChats/' + currentUser.uid + '/' + chatId).remove())
            .then(() => { showNotification('Вы отписались', 'info'); closeChannelProfileModalSimple(); loadChats(); });
    } else {
        database.ref('chats/' + chatId + '/subscribers/' + currentUser.uid).set(true)
            .then(() => database.ref('userChats/' + currentUser.uid + '/' + chatId).set(true))
            .then(() => { showNotification('Вы подписались!', 'success'); closeChannelProfileModalSimple(); });
    }
}
...
function closeChannelProfileModalSimple() {
    const modal = document.getElementById('channel-profile-modal-simple');
    if (modal) modal.remove();
}
